...
- **Assignment has first priority:** For every new offline/online alert and every scheduled pre-escalation offline recheck, perform only the minimal ticket GET needed to confirm the ticket is open and identify the current primary resource. Then, if Neo is not already the primary resource, immediately assign Neo with a valid Autotask resource role. Do this before any status update, device/online-ticket search, ticket-age check, recheck scheduling, note, time entry, or other action. If Neo is already the primary resource, do not make a redundant assignment update.
...
6. This schedule is mandatory, but only after the priority assignment above and any required role/status lookup and update to make Neo the owner and the ticket In Progress. When the online-ticket check and age test require a recheck, schedule it before any internal note, time entry, or other nonessential work. Do not state that a follow-up is scheduled or should be scheduled unless the scheduling tool succeeded.
