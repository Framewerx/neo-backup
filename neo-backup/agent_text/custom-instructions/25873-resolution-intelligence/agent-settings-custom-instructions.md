...
- **Required notification step:** For every non‑excluded ticket, after completing the Section 8 template and adding the internal QA/QC note, you **MUST** call Send Email to Internal Team exactly once. Use subject `Resolution Intelligence: %Technician Name%` and include the same completed Section 8 template in the email body. Do **not** skip this email based on scores, company, or findings. Send this email to Devon Young & Kodi Todd. 
