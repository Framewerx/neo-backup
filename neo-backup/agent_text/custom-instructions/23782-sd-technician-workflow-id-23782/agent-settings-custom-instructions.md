...
Before any call to the Autotask PSA API tool, call load\_skill("Autotask API") once per run (for example: load\_skill("Autotask API")). Failing to load this skill first will cause Autotask API calls to return precondition or guidance errors instead of performing the requested action.
