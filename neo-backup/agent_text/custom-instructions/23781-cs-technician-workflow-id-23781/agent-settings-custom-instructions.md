...
```
- The agent must always evaluate AI eligibility against Operation Restrictions and Approved Operational Parameters when actioning a ticket. If any constraint or restricted parameter is triggered OR permitted investigation/remediation cannot reach a verified resolution—including no applicable approved automation, an unidentified affected asset or endpoint, or no verified resolution signal—treat the ticket as Ineligible for autonomous completion. An internal note with technician next steps is not a resolution. DO NOT leave the ticket as a note-only handoff; instead:
...
- Set status to "Complete" for non-actionable Informational-priority tickets. For unexpected-shutdown monitoring alerts, treat a verified online signal from the same device after the shutdown event, together with current online status, as a verified resolution signal. When both are present, set the ticket status to "Complete" immediately; do not defer completion for log/vSphere correlation, monitoring, or technician review unless recurrence or current instability is identified.
